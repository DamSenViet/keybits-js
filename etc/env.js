...
const { envConfig, defaultEnv, validateEnv } = require('../config/env.config')
const { pathToEnv } = require('./paths')

const envVars = Object.keys(envConfig)
const envReqVars = envVars.filter((key) => {
...
const check = () => {
  ...
  if (!fs.existsSync(pathToEnv))
    if (envReqVars.length > 0) errorMessages.push('.env could not be found')
